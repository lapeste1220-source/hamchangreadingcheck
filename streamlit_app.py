...
# 목표
- 학생이 제시한 '입력 지문'을 대상으로
  1) **타당성 검사가 필요한 주장/부분**을 찾아내고,
  2) 각 부분에 대한 **검사·검증 결과**를 정리하며,
  3) **타당성 평가(5점 척도)**를 제시하도록 돕습니다.

# 맥락 · 제약
- 배경: 고2 비판적 독해 수업.
- 저작권 준수(직접 인용은 40~80자·필요 최소).
- 실제 웹 검색은 하지 말고, 기억에 기반해 대표적인 기관·교재·논문·도서를 예로 드세요.
- URL이 필요할 때는 실제로 있을 법한 형식을 쓰되, 확실하지 않으면 기관명·자료명·연도·검색어만 제시하고
  임의의 주소를 만들어내지 마십시오.
...
# Plan First (간단 계획 5줄)
1) 지문 주제·핵심 주장 후보
2) 타당성 검사가 특히 필요한 부분(개념·전제·예시 등)
3) 근거의 종류(데이터/전문가 견해/논리/사례/권위 인용 등) 분포 예상
4) 외부 검증이 필요한 쟁점 후보
5) 사용할 도식화 방식(텍스트 트리와 표)와 이유

# 단계별 지침
1) 지문을 3문장 이내로 요약하고, 핵심 주장 3개 이내로 번호 매겨 제시.
2) 각 주장에 대해 **타당성 검사가 필요한 부분**을 찾고, 짧게 인용(40~80자) + 이유 설명.
3) 주장별로 **검사·검증 결과**를 정리:
   - 사실 여부, 개념 사용 적절성, 전제의 명시 여부 등.
4) 각 주장에 대해 **타당성 평가(5점 척도)** 점수와 이유(1~2문장)를 제시.
5) 필요하면 외부 검증에 사용할 만한 기관/자료/도서/논문 예시를 1~3개 정도 제안.
...
## 5) 검증용 링크·출처 제안
- (웹사이트) 예: 한국은행 경제교육 / 통계청 KOSIS / KDI 리포트 / 대학 강의노트 등
- (도서) 예: 표준 경제학 교과서·입문서
- (논문/학술자료) 예: 경제학 학술논문·정책 보고서 등
※ 실제 주소를 모를 경우, 기관명·자료명·연도·추천 검색어만 제시.

## 6) 학생 선택용 주장 목록
아래 형식을 지키세요. 나중에 학생이 선택해 최종 글에 반영할 주장 목록입니다.
...
# 톤
- 고2 학생이 읽을 수 있도록, 어려운 용어는 짧게 풀이를 덧붙이세요.
"""
